...
// FIXME: Set appropriate chunk/buffer size. This might be too small.
...
impl FileOpener for S3SelectOpener {
    fn open(
        ...
        // ScanRange doesn't support compressed data.
        let use_scan_range = file_compression_type == FileCompressionType::UNCOMPRESSED;
        let chunk_size = match use_scan_range {
            true => chunk_size,
            false => file_size,
        };
            ...
            let res = futures::stream::iter(chunks.tuple_windows::<(usize, usize)>())
                .map(|(start, end)| {
                    ...
                    tokio::spawn(async move {
                        ...
                        if use_scan_range {
                            request = request.scan_range(
                                ScanRange::builder().start(start).end(end).build(),
                            )
                        }

                        let custom_request = request.customize().await?;
                        ...
                        //        Once fixed, remove this logic.
                        //        https://github.com/awslabs/aws-sdk-rust/issues/630
                        let custom_request = custom_request
                            .map_request::<DataFusionError>(|x| {
                                Ok(x.map(|y| {
                                    y.map(|z| {
                        ...
                        let mut response = custom_request.send().await?;
